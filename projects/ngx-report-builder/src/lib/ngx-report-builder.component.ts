...
import {
  ...
  RuleSet,
  EmptyWarningContext,
} from './interfaces/ngx-report-builder.interfaces';
import {
  ChangeDetectorRef,
  Component,
  ContentChild,
...
export const CONTROL_VALUE_ACCESSOR: any = {
  ...
  useExisting: forwardRef(() => NgxReportBuilderComponent),
  multi: true,
};
  ...
  implements OnInit, OnChanges, ControlValueAccessor, Validator
{
  ...
  @Input() data: any | RuleSet = { condition: 'and', rules: [] };

  // For ControlValueAccessor interface
  public onChangeCallback: (() => void) | undefined;
  public onTouchedCallback: (() => any) | undefined;
  ...
  @Input() classNames: QueryBuilderClassNames | undefined;
  @Input() operatorMap: { [key: string]: string[] } | undefined;
  @Input() parentValue: RuleSet | undefined;
  @Input() config: QueryBuilderConfig = { fields: {} };
  @Input() parentArrowIconTemplate: QueryArrowIconDirective | undefined;
  @Input() parentInputTemplates: QueryList<QueryInputDirective> | undefined;
  @Input() parentOperatorTemplate: QueryOperatorDirective | undefined;
  @Input() parentFieldTemplate: QueryFieldDirective | undefined;
  @Input() parentEntityTemplate: QueryEntityDirective | undefined;
  @Input() parentSwitchGroupTemplate: QuerySwitchGroupDirective | undefined;
  @Input() parentButtonGroupTemplate: QueryButtonGroupDirective | undefined;
  @Input() parentRemoveButtonTemplate: QueryRemoveButtonDirective | undefined;
  @Input() parentEmptyWarningTemplate: QueryEmptyWarningDirective | undefined;
  @Input() parentChangeCallback: (() => void) | undefined;
  @Input() parentTouchedCallback: (() => void) | undefined;
  @Input() persistValueOnFieldChange: boolean = false;

  @ViewChild('treeContainer', { static: true }) treeContainer:
    | ElementRef
    | undefined;

  @ContentChild(QueryButtonGroupDirective) buttonGroupTemplate:
    | QueryButtonGroupDirective
    | undefined;
  @ContentChild(QuerySwitchGroupDirective) switchGroupTemplate:
    | QuerySwitchGroupDirective
    | undefined;
  @ContentChild(QueryFieldDirective) fieldTemplate:
    | QueryFieldDirective
    | undefined;
  @ContentChild(QueryEntityDirective) entityTemplate:
    | QueryEntityDirective
    | undefined;
  @ContentChild(QueryOperatorDirective) operatorTemplate:
    | QueryOperatorDirective
    | undefined;
  @ContentChild(QueryRemoveButtonDirective) removeButtonTemplate:
    | QueryRemoveButtonDirective
    | undefined;
  @ContentChild(QueryEmptyWarningDirective) emptyWarningTemplate:
    | QueryEmptyWarningDirective
    | undefined;
  @ContentChildren(QueryInputDirective) inputTemplates:
    | QueryList<QueryInputDirective>
    | undefined;
  @ContentChild(QueryArrowIconDirective) arrowIconTemplate:
  ...
  private operatorsCache: { [key: string]: string[] } | undefined;
  ...
  private buttonGroupContext: ButtonGroupContext | undefined;
    ...
    if (type === 'object') {
      ...
      if (config.entities) {
        this.entities = Object.keys(config.entities).map((value) => {
          ...
          if (config.entities && value) {
            entity = config.entities[value];
  ...
  findTemplateForRule(rule: any): TemplateRef<any> | null {
    const type: string | null = this.getInputType(rule.field, rule.operator);
    if (type) {
  ...
  getOperators(field: string): string[] {
    ...
    if (this.operatorsCache) this.operatorsCache[field] = operators;
    return operators;
  }
  ...
  getClassNames(...args: string[]): string | null {
    const clsLookup: QueryBuilderClassNames = this.classNames
      ? this.classNames
      : this.defaultClassNames;
    if (clsLookup) {
      //TODO: Validate This
      const classNames: any = args
        .map((id) =>
  ...
  addRule(parent?: any): void {
    ...
    if (this.config.addRule) {
      ...
      parent.rules.concat(newruleset);
    }
  ...
  removeRule(rule: any, parent?: any): void {
    if (this.disabled) {
      return;
    }
  ...
  removeRuleSet(ruleset?: any, parent?: any): void {
    ...
    if (this.config.removeRuleSet) {
      if (parent) this.config.removeRuleSet(ruleset, parent);
    } else {
      if (parent) parent.rules = parent.rules.filter((r: any) => r !== ruleset);
    }
  ...
  changeField(fieldValue: string, rule: any): void {
    if (this.disabled) {
      return;
    }
  ...
  getButtonGroupContext(): ButtonGroupContext | undefined {
    if (!this.buttonGroupContext && this.allowRuleset && this.parentValue) {
  ...
  getRemoveButtonContext(rule: any): RemoveButtonContext | undefined {
  ...
  getFieldContext(rule: any): FieldContext | undefined {
  ...
  getEntityContext(rule: any): EntityContext | undefined {
  ...
  getOperatorContext(rule: any): OperatorContext | undefined {
  ...
  getInputContext(rule: any): InputContext | undefined {
